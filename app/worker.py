...
from storage import is_sent, mark_sent
...
def process_feed(feed_url):
    ...
    for item in items:
        ...
        item.content_html = build_description(item)
        result = send(item)
        mark_sent(item.id)
...
def build_description(item):
    ...
    description = html_to_text(item.content_html or "").strip()

    # Fallback
    if not description:
        description = "Nouvel article disponible. Cliquez pour en savoir plus..."
